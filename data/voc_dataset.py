    ...
    def get_example(self, i):
        ...
        '''for obj in anno.findall('object'):
            ...
            bbox.append([
                ...
                for tag in ('ymin', 'xmin', 'ymax', 'xmax')])
            name = obj.find('name').text.lower().strip()
            label.append(MRI_LABEL_NAMES.index(name))'''

        #print(metadata["volumeFilename"])
        #print(metadata["volumeFilename"] == id_)

        #id_ = id_[:-2]

        exams = metadata[metadata["volumeFilename"] == id_ + ".pck"]
        exam = exams[0]
        ...
        ran_index = random.randrange(z_start, z_start + depth)
        volumetric_data = volumetric_data[ran_index, :, :]
        ...
        #np.save("imge.png", img)

        assert img.shape[0] == 3 and img.shape[1] == 320 and img.shape[2] == 320
